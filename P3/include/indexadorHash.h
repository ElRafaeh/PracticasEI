...
class IndexadorHash {
    public:
        ...
        IndexadorHash(const string& directorioIndexacion);
            /* Constructor para inicializar IndexadorHash a partir de una indexaci?n previamente realizada que habr? sido almacenada en
            ?directorioIndexacion? mediante el m?todo ?bool GuardarIndexacion()?. Con ello toda la parte privada se inicializar? convenientemente, igual
            que si se acabase de indexar la colecci?n de documentos. En caso que no exista el directorio o que no contenga los datos de la indexaci?n se
        ...
        bool Indexar(const string& ficheroDocumentos);
            ...
            true, mostrando el mensaje de excepci?n correspondiente, y se re-
            indexar?n (borrar el documento previamente indexado e indexar el nuevo)
        ...
        bool GuardarIndexacion() const;
            ...
            a.GuardarIndexacion();?, entonces mediante el comando: ?IndexadorHash b(?./dirIndexPrueba?);? se
            recuperar? la indexaci?n realizada en la secuencia anterior, carg?ndola en ?b?
        ...
        void ImprimirIndexacion() const;
            ...
            imprimir?: cout << nomDoc << '\t' << InfDoc << endl;*/
        ...
        bool Existe(const string& word) const;
            /* Devuelve true si word (aplic?ndole el tratamiento de stemming y
            may?sculas correspondiente) aparece como t?rmino indexado */
        ...
        bool BorraDoc(const string& nomDoc);
            ...
            // Borra todos los t?rminos del ?ndice de documentos: toda la indexaci?n
            de documentos. */
        void VaciarIndicePreg();
            /* Borra todos los t?rminos del
            indexaci?n de la pregunta actual.
            ?ndice de la pregunta: toda */
        ...
        int NumPalIndexadas() const;
            // Devolver? el n?mero de t?rminos diferentes indexados (cardinalidad de
            // campo privado ??ndice?)
        ...
        void ListarTerminos() const;
            ...
            // ??ndice?: cout << termino << ?\t? << InformacionTermino << endl;
        ...
        IndexadorHash();
            ...
            crear un indexador sin inicializarlo convenientemente. La inicializaci?n
            la decidir? el alumno */
        ...
        InformacionPregunta infPregunta;
            // Informaci?n recogida de memoria principal en la pregunta indexada. Se almacenar? memoria principal
        ...
        bool almacenarPosTerm; 
            // Si es true se almacenar? la posici?n en la que aparecen los t?rminos
            // dentro del documento en la clase InfTermDoc
        void GuardarStopWords(const string &);
            // M?todo auxiliar que lee el ficheroStopWords y guarda las palabras de parada
            // en stopWords
        bool IndexarDocumento(const string &documento, int idDoc = 0);
            // Devuelve true si consigue indexar el documento. Guarda la informaci?n a indexar del documento

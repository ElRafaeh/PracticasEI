...
class Buscador: public IndexadorHash {
    ...
    public:
        ...
        bool Buscar(const int& numDocumentos = 99999, const int &nump=0);
            ...
            // esta variable antes de realizar la búsqueda). Se almacenarán solo los
            // documentos que compartan algún término (no de parada) con la query
            // (aunque ese número de documentos sea inferior a ?numDocumentos?). Como
            // número de pregunta en ?ResultadoRI.numPregunta? se almacenará el valor
            // 0. En caso de que no se introduzca el parámetro ?numDocumentos?,
        ...
        void ImprimirResultadoBusqueda(const int& numDocumentos = 99999) const;
            ...
            // número MÁXIMO de ?numDocumentos? (en caso de que no se introduzca el
            // parámetro ?numDocumentos?, entonces dicho parámetro se inicializará a
            // 99999) por cada pregunta, entre los que tienen algún término de la
            // pregunta), los cuales estarán almacenados en la variable privada
            // ?docsOrdenados? en orden decreciente según la relevancia sobre la
            // pregunta, en el siguiente formato (una línea por cada documento):
            ...
            frelevancia_trec_eval_TIME.txt
            fich_salida_buscador.txt
            >

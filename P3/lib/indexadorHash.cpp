...
void IndexadorHash::GuardarStopWords(const string &fichStopWords)
{
    ...
    if(fich.is_open())
    {
        ...
        {
            this->stopWords.insert(aplicarStemYMayus(stopWord));
        }
        fich.close();
    }
    else
...
// Constructor a ra�z de una indexaci�n previa
...
IndexadorHash::~IndexadorHash()
{
    ...
    this->indicesDisco = 0;
    this->indicesDocsDisco = 0;
    this->indicesEnDisco.clear();
    this->indicesDocsEnDisco.clear();
}
...
// Funci�n para sacar el tama�o de memoria utilizado por 
...
bool IndexadorHash::IndexarDocumento(const string &documento, int idDoc)
{
    ...
    if(fd != -1)
    {
        ...
        int len = lseek(fd, 0, SEEK_END);   // len tambi�n es el tama�o del documento
        ...
        // Sacamos informaci�n del documento
        ...
        for(it = tokens.begin(); it != tokens.end(); it++)
        {
            ...
            if(stopWords.find(*it) == stopWords.end())  // Verificamos que no el stem no es una stopword
            {
                ...
                if(iteradorIndice == indice.end()) // Si el termino no esta indexado
                {
                    ...
                    // Inicializamos la informaci�n del termino en el documento
                    ...
                    // Actualizamos la informaci�n del termino
...
bool IndexadorHash::Indexar(const string &ficheroDocumentos)
{
    ...
    if(fd != -1)
    {
        ...
        // Estructura stat para sacar la fecha de modificaci�n de los documentos
        struct stat infoDoc;
        
        // Cerramos el documento
        close(fd);

        // Leemos l�nea a l�nea del fichero
        while(getline(ss, documento, '\n'))
        {
            ...
            if(itIndiceDocumento != indiceDocs.end())
            {
                ...
                {
                    // Eliminamos la indexaci�n del fichero antiguo
                    if(!BorraDoc(documento))
                    {
                        cerr << "No se ha podido borrar la indexaci�n antigua del documento: " << documento << endl;
                        return false;
                    }
            ...
            // Si la variable almacenarEnDisco est� a true, almacenamos la indexacion en el disco duro
            if(almacenarEnDisco)
            {
                ...
                // Si la memoria que estamos usando es mayor o igual a la memoria total del sistema vamos a guardar los indices en memoria principal
                if(physMemUsed >= 10000)
                {
                    cout << "Mem: " << physMemUsed << endl;
...
// Metodo para guardar la indexaci�n en el disco duro
...
// M�todo para recuperar la indexaci�n guardada en el disco duro
bool IndexadorHash::RecuperarIndexacion(const string &directorioIndexacion)
{
    ...
    if(fd != -1)
    {
        ...
        // Cogemos la informaci�n de infPregunta
        getline(ss, aux, '\n');
...
// M�todo para imprimir la indexacion
...
// M�todo que indexa la informacion de la pregunta
bool IndexadorHash::IndexarPregunta(const string &preg)
{
    ...
    if(!preg.empty())
    {
        ...
        {
            cerr << "Pregunta con todo t�rminos de parada" << endl;
            return false;
        }
        else
        {
    ...
    cerr << "La pregunta no tiene t�rminos" << endl;
    return false;
}
...
// Devuelve el n�mero de palabras indexadas
...
// Devuelve el n�mero de palabras de parada
...
// Muestra por pantalla los t�rminos indexados
...
// Muestra por pantalla los t�rminos indexados del documento nomDoc
...
// Metodo para aplicar stemming y pasar a min�sculas una palabra
